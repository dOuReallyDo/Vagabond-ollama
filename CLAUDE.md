...
- **Nominatim geocoding** (`src/lib/nominatim.ts`): After Zod validation in both `generateItinerary()` and `modifyItinerary()`, resolves `mapPoints`, `attractions`, and `activities` location names to accurate OSM coordinates via Nominatim API. Falls back to AI-generated coordinates if Nominatim fails. Free tier, 1 req/sec rate limiter, in-memory cache, no API key needed.
  ...
  - **`CITY_NAME_MAP`**: 80+ Italian→local city name mappings for better Nominatim results. European: "lisbona"→"Lisbon", "marsiglia"→"Marseille", "monaco"→"Munich" (Monaco di Baviera, not Principato), "parigi"→"Paris", "siviglia"→"Seville", "atene"→"Athens", etc. **Asian entries** (May 2026): Komodo, Bali, Ubud, Jakarta, Labuan Bajo, Flores, Lombok, Gili Islands, Tokyo, Kyoto, Osaka, Hiroshima. **Cape Verde entries** (May 2026): 'boa vista'→'Boa Vista, Cape Verde', 'sal rei', 'sal', 'santa maria', 'santo antão', 'sao vicente', 'mindelo', 'santiago', 'praia', 'fogo', 'maio'. Without this, ambiguous Italian names can geocode to wrong countries.
  ...
  - **`countryMap` expansion** (May 2026): Now 70+ countries including Indonesia, Malaysia, Philippines, Japan, Korea, NZ, SE Asia, India, Sri Lanka, Myanmar, Cambodia, Singapore, South Africa, **Cape Verde (cv)**, Kenya, Tanzania, Mozambique, Senegal, Ghana, Nigeria, Ethiopia, Tunisia, Algeria, Cuba, Dominican Republic, Jamaica, Costa Rica, Panama, UAE, Oman, Qatar, Jordan, Lebanon, Taiwan, Hong Kong, Macao. Previously only had ~30 European/Americas countries, causing `detectCountryCode()` to fail for destinations like "Boa Vista (Capo Verde)".
  ...
  - **Dynamic proximity threshold** (May 2026): Was fixed 50km, now varies by country type:
    - **Archipelago nations** (ID, PH, PG, NZ, JP, MY, LK, **CV**): 500km — islands are far apart (e.g. Ubud to Komodo = 460km, Boa Vista to Santiago CV = 200km)
    - **Large countries** (US, CN, AU, BR, IN, RU, CA, MX, AR): 300km
    - **Default** (Europe/medium): 50km — works for Italian "frazioni" disambiguation
  - **`destCoords` validation against AI mapPoints** (May 2026): After geocoding the main destination, compare with the centroid of AI-generated mapPoints. If >1000km apart, Nominatim likely geocoded the wrong country (e.g. "Boa Vista" → Brazil instead of Cape Verde). Use AI center as reference instead. Also fallback to AI center if destCoords is null. This is a critical safety net — it prevents the entire proximity check system from being poisoned by a wrong destination reference.
...
- **`countryMap` expansion**: Now 70+ countries including Indonesia, Malaysia, Philippines, Japan, Korea, NZ, SE Asia, India, Sri Lanka, Myanmar, Cambodia, Singapore, South Africa, **Cape Verde (cv)**, Kenya, Tanzania, Mozambique, Senegal, Ghana, Nigeria, Ethiopia, Tunisia, Algeria, Cuba, Dominican Republic, Jamaica, Costa Rica, Panama, UAE, Oman, Qatar, Jordan, Lebanon, Taiwan, Hong Kong, Macao. Previously only ~30 European/Americas countries, so `detectCountryCode()` failed for many destinations.
...
- **Dynamic proximity threshold**: Was fixed 50km, now varies by country type:
  - Archipelago nations (ID, PH, PG, NZ, JP, MY, LK, **CV**): 500km — islands are far apart (e.g. Ubud to Komodo = 460km)
  - Large countries (US, CN, AU, BR, IN, RU, CA, MX, AR): 300km
  - Default (Europe/medium): 50km — works for Italian "frazioni" disambiguation
- **`destCoords` validation against AI mapPoints** (May 2026): After geocoding the main destination, compare with the centroid of AI-generated mapPoints. If >1000km apart, Nominatim likely geocoded the wrong country (e.g. "Boa Vista" → Brazil instead of Cape Verde). Use AI center as reference instead. Critical safety net — prevents the entire proximity check system from being poisoned by a wrong destination reference.
- **`CITY_NAME_MAP` expanded**: 80+ entries. Asian: Komodo, Bali, Ubud, Jakarta, Labuan Bajo, Flores, Lombok, Gili Islands, Tokyo, Kyoto, Osaka, Hiroshima. **Cape Verde**: 'boa vista' → 'Boa Vista, Cape Verde', 'sal rei', 'sal', 'santa maria', 'santo antão', 'sao vicente', 'mindelo', 'santiago', 'praia', 'fogo', 'maio'.
- **`stripItalianPrefix()` expanded** (May 2026): 50+ patterns. Added: abbazia, basilica, chiesa, cattedrale, duomo, faro, spiaggia, riserva, oasi, santuario, monumento, area marina protetta, fondamenta, portico, campus, piazzale, riva. Fixes "Abbazia di San Giorgio Maggiore" → "San Giorgio Maggiore", "Riserva Naturale Alberoni" → "Alberoni".
...
│   ├── nominatim.ts                # Nominatim geocoding — stripItalianPrefix() (50+ regex), CITY_NAME_MAP (80+ incl. Asian + Cape Verde), extractPlaceName() pipeline, countryMap (70+ incl. Cape Verde + Africa + Middle East + Caribbean), destCity parenthesized country cleanup, dynamic proximity threshold (archipelago 500km / large 300km / default 50km), destCoords validation against AI mapPoints center (>1000km → use AI center), country code fallback from departureCity, Step 4 city extraction override, rate limiter + cache
