...
│   ├── nominatim.ts                # Nominatim geocoding (cache-aware: mapPoints strip prefixes, activities use location, attractions name+destination fallback; Step 4 city extraction override; rate limiter + cache, free tier, 1 req/sec) — no estimateDriveDurationMinutes (removed)
