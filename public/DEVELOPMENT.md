...
- **preferredStops**: `inputs.preferredStops` (1-10, opzionale) nel form e nel prompt. Quando impostato, l'AI usa ESATTAMENTE quel numero di tappe, distribuendo le notti (~giorni/tappe notti per tappa). Quando non impostato, fallback automatico a N/2. **Minimo 2 notti per tappa sempre enforced**.
- **Distribuzione tappe**: Il prompt impone "REGOLE PER LA DISTRIBUZIONE DELLE TAPPE" — ogni tappa ALMENO 2 notti, max N/2 tappe per viaggio di N giorni (se `preferredStops` non impostato), altrimenti esattamente il numero scelto dall'utente. Città principali 2-3 notti, modello base+escursione. Il prompt compatto include la stessa regola.
...
- **Nominatim geocoding (post-processing)**: Dopo che l'AI genera l'itinerario, `geocodeItinerary()` (da `src/lib/nominatim.ts`) risolve i nomi di località in coordinate lat/lng accurate sostituendo quelle fornite dall'AI. Geocodifica **cache-aware**: (1) `mapPoints` — geocodifica usando solo il nome della città/luogo, stripping i prefissi descrittivi italiani come "Escursione a", "Visita a", "Tour a", "Giornata a", ecc.; (2) attrazioni — prova prima con il `name` da solo, poi fallback `name + destination`; (3) attività — geocodifica usando solo il campo `location` (nome città), non il nome dell'attività. Lookup duplicati della stessa città sono saltati (cache hit). Se Nominatim fallisce per un punto, mantiene le coordinate AI come fallback. Vedi anche Regola Critica #26.
...
- **Programmatic car segments**: Quando `flightPreference` include "auto", `generateCarSegments()` crea un segmento per tratta (partenza→tappa1→...→ritorno) — **nessuna chiamata AI per auto**. **Singola opzione per segmento** con costo stimato carburante+pedaggi (€0.15/km + €0.07/km) e URL Google Maps. **Niente km/durata** (commit 5bac49d): i segmenti auto passano `distance: null` e `duration: null` — mostrano solo costo stimato e nota "Distanza e durata: consulta Google Maps per il percorso reale". La funzione `estimateDriveDurationMinutes` è stata rimossa. **Stima distanza a 3 livelli**: (1) tabella 80+ rotte europee hardcodate (`estimateRoadKm`), (2) fallback Nominatim geocoding + haversine (`estimateRoadKmFromCoords` da `src/lib/nominatim.ts`, fattore stradale 1.35×), (3) ultimo resort 400km. FlightCard usa `flight.airline.toLowerCase().includes('auto privata')` per renderizzare segmenti auto con Google Maps iframe.
...
|| `src/lib/nominatim.ts` | Geocoding Nominatim centralizzato — `geocodeItinerary()` (Step 1 post-processing, cache-aware: mapPoints strip prefixes, activities use location, attractions name→name+destination fallback), `geocodeCities()` + `estimateRoadKmFromCoords()` (Step 2 fallback distanze auto), `haversineKm()`, cache + rate limiter 1 req/sec |
...
13. **Distribuzione tappe** — il prompt Step 1 impone ogni tappa ALMENO 2 notti. Se `preferredStops` è impostato, usa esattamente quel numero; altrimenti max N/2 tappe per viaggio di N giorni. Se l'AI genera 10 tappe per 10 giorni, è un bug del prompt.
...
23. **Car route: generazione programmatica (singola opzione, no km/durata)** — Quando `flightPreference` include "auto", `generateCarSegments()` crea segmenti in JS puro (nessuna chiamata AI). Un segmento per tratta (partenza→tappa1→tappa2→...→ritorno). **Singola opzione** per segmento (rimosse le 2 opzioni Autostrada/Senza pedaggi). Stima distanza: tabella 80+ rotte europee → fallback Nominatim+Haversine (1.35× fattore stradale) → 400km ultimo resort. Costo: €0.15/km carburante + €0.07/km pedaggi. **Niente km/durata** (commit 5bac49d): `generateCarSegments` passa `duration: null` e `distance: null` — mostra solo costo stimato e nota "Distanza e durata: consulta Google Maps per il percorso reale". La funzione `estimateDriveDurationMinutes` è stata rimossa. Ogni segmento ha URL Google Maps (`flight.bookingUrl`). FlightCard usa `flight.airline.toLowerCase().includes('auto privata')`.
...
27. **Nominatim rate limiting e geocoding cache-aware** — Il modulo `src/lib/nominatim.ts` implementa `rateLimitedFetch()` con intervallo minimo 1.1 secondi tra richieste e `User-Agent` header richiesto. È VIETATO chiamare `https://nominatim.openstreetmap.org/search` direttamente senza passare attraverso il rate limiter. La policy Nominatim (https://operations.osmfoundation.org/policies/nominatim/) impone max 1 req/sec. Le chiamate sono cachate in memoria per sessione (`geocodeCache`). Per Step 1, `geocodeItinerary()` processa mapPoints in parallelo (ma rispettando il rate limiter); per Step 2, `geocodeCities()` è usata come fallback per distanze non nella tabella rotte europee. **Geocoding logica cache-aware (commit 70df59e)**: mapPoints — stripping prefissi descrittivi italiani ("Escursione a", "Visita a", "Tour a", "Giornata a") prima di geocodificare; activities — usa solo il campo `location` (nome città), non il nome dell'attività; attractions — prova `name` da solo, poi fallback `name + destination`; lookup duplicati della stessa città sono saltati (cache hit).